...
class InsMem(object):  # read instruction
    def __init__(self, name, ioDir):
        ...
        try:
            ...
            raise FileNotFoundError(f"Instruction memory file (imem.txt) not found. Please add it to: {path}")
...
class DataMem(object):
    def __init__(self, name, ioDir):
        self.id = name
        self.ioDir = ioDir
        path = os.path.join(ioDir, "dmem.txt")
        try:
            with open(path) as dm:
                self.DMem = [line.strip() for line in dm.readlines()]
        except FileNotFoundError:
            raise FileNotFoundError(f"Data memory file (dmem.txt) not found. Please add it to: {path}")
...
class RegisterFile(object):
    def __init__(self, ioDir):
        ...
        self.registers = [int2bin(0) for _ in range(32)] # fiveStage case
